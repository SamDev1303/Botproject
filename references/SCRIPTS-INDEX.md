# Scripts Index — Quick Reference for Bella

> Every script in `~/Desktop/🦀/scripts/` mapped by task category
...
|--------|-------------|-------|
| `morning_brief.py` | Daily summary: unpaid invoices, payments, dashboard stats | `python3 scripts/morning_brief.py` |
| `check_inbox.py` | List unread Gmail messages | `python3 scripts/check_inbox.py [limit]` |
| `weekly_report.py` | Weekly P&L, revenue, outstanding summary | `python3 scripts/weekly_report.py` |
...
| `bookkeeping_helpers.py` | GST, super, expense categories, P&L helpers | Library — imported by other scripts |
| `square_sheets_sync.py` | Sync Square payments → Google Sheets | `python3 scripts/square_sheets_sync.py` |
| `sync_square_sheets.sh` | Shell wrapper for above with logging | `bash scripts/sync_square_sheets.sh` |
...
| Script | What it does | Usage |
|--------|-------------|-------|
| `google_sheets_api.py` | Google Sheets API wrapper (read, append, update) | Library — import GoogleSheetsAPI |
| `google-oauth-setup.py` | One-time OAuth token setup for Google | `python3 scripts/google-oauth-setup.py` |
...
| Script | What it does | Usage |
|--------|-------------|-------|
| `api_health_check.py` | Test all configured APIs without exposing creds | `python3 scripts/api_health_check.py` |
...
## MCP Servers (15 servers, 160+ tools)
